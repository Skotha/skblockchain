...
contract PausableToken is StandardToken, Pausable {
  ...
  function blackListAddress(address listAddress,  bool isBlackListed) public whenNotPaused onlyTreasurer  returns (bool success) {
    ...
    function viewDiscountCoupon(address _account) public view returns (uint256){
       return discountCoupon[_account];
    }

  
    function discountedTransfer(address _to, uint256 _value) public returns (uint256) {
        uint256 amount;
        //amount = _value*(1-(discountCoupon[_from]/100));
        amount = _value.sub(discountCoupon[msg.sender]);
